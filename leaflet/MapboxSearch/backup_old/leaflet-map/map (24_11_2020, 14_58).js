      ...
      success: function () {
        ...
        if (
          ...
          $(document).ready(function () {
            ...
            let getDate
            let getPeople
            let getAirco, getPool, getGym, getSauna, getParking, getNature
            let availableHotels = []
            let geoDatePeople = []
            let objectKeyName = []
            let i, t
    		...
    		*/
            ...
            let filterDates = () => {
              ...
              $.each(availability[0], function (index) {
                ...
                if (this[diffDays] > 0 && obj.dayPrices[getPeople - 1] > 0) {
                  ...
                  if (bookingEnd === "TRUE" && diff_in_days === 1) {
                    ...
                    if (currentDate < latestBookingTime) {
                      ...
                      return "Booking end time not exceeded"
                    }
                  }
                  // If there's a booking end time and booking day is today
                  if (bookingEnd === "TRUE" && diff_in_days === 0) {
                    
                    var earliestBookingDate = new Date()
                    earliestBookingDate.setHours(16,0,0,0);
                    console.log("earliestBookingDate " + earliestBookingDate)
                    console.log("currentDate " + currentDate)

                    if (currentDate > earliestBookingDate) {
                      console.log("boom")
                      availableHotels.push(index)
                      return "Booking start time not preceeded"
                    }
                  }
                  console.log("still in")
                  availableHotels.push(index)
                }
                if (this[diffDays] = 0) { //&& obj.dayPrices[getPeople - 1] > 0 && bookingEnd === "TRUE"
                  console.log("inn")
                }
            ...
            let cityFocus = () => {
              ...
              if (!getCity) {
                ...
                citySplit = getCity.split("~")
                map.setView(citySplit, 13)
              }
            }
